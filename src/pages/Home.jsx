...
const App = () => {
  ...
  gap-[16px] rounded-[10px] bg-white">
              <div className='flex flex-col gap-[9px] sm:gap-[16px] w-full justify-center'>
                  <div className='flex flex-row justify-start items-end sm:grid gap-[12px] sm:gap-[16px] '>

                  <div className="flex items-center justify-center h-fit">
                  <img className='w-[90px] sm:w-[344px] items-center aspect-square sm:aspect-16/9 object-cover rounded-[10px]' src={randomImage} alt="product" />
                  </div>

                  <div className='flex h-full flex-col gap-[8px] sm:gap-[16px]'>
                  <div className='flex flex-col gap-[8px]'>
                  <HeadingSemiBold size="card" className="text-left cursor-pointer">{e.title}</HeadingSemiBold>
                  <BodyMedium className="hidden sm:block sm:text-base text-dark-secondary">{e.description.length > 65 ? e.description.slice(0, 65) + '...' : e.description}</BodyMedium>
                  </div>
                  ...
                  <div className='flex flex-row items-center justify-between w-full'>
                      ...
                      <img src={ratings} alt="" />
                      <BodyMedium className="text-xs sm:text-sm text-dark-secondary underline whitespace-nowrap">{`${e.avg_rating} (${e.total_reviews})`}</BodyMedium>
                      </div>
                      <HeadingSemiBold size="price" className="text-primary whitespace-nowrap">{`Rp ${e.price}K`}</HeadingSemiBold>
                  </div>
              </div>
              </div>
          )
